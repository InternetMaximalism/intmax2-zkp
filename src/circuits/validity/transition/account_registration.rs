...
impl AccountRegistrationTarget {
    ...
    pub fn to_buffer(&self, buffer: &mut Vec<u8>) -> IoResult<()> {
        ...
        buffer.write_usize(self.account_registration_proofs.len())?;
        for account_registration_proof in &self.account_registration_proofs {
            account_registration_proof.to_buffer(buffer)?;
        }
    ...
    pub fn from_buffer(buffer: &mut Buffer) -> IoResult<Self> {
        ...
        let account_registration_proofs_len = buffer.read_usize()?;
        let mut account_registration_proofs = Vec::with_capacity(account_registration_proofs_len);
        for _ in 0..account_registration_proofs_len {
            account_registration_proofs.push(AccountRegistrationProofTarget::from_buffer(buffer)?);
        }
    ...
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    pub fn from_buffer(
        ...
        let target = AccountRegistrationTarget::from_buffer(buffer)?;
