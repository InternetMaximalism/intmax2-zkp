...
{
    pub fn new(claim_inner_verifier_data: &VerifierCircuitData<F, C, D>) -> Self {
        let mut builder = CircuitBuilder::<F, D>::new(CircuitConfig::default());
        let is_first_step = builder.add_virtual_bool_target_safe();
        let is_not_first_step = builder.not(is_first_step);
        let claim_inner_proof =
            add_proof_target_and_verify(claim_inner_verifier_data, &mut builder);
        let prev_claim_hash =
            Bytes32Target::from_slice(&claim_inner_proof.public_inputs[0..BYTES32_LEN]);
        let claim_hash =
            Bytes32Target::from_slice(&claim_inner_proof.public_inputs[BYTES32_LEN..]);
        ...
        prev_pis.connect(&mut builder, prev_claim_hash);
        // initial condition
